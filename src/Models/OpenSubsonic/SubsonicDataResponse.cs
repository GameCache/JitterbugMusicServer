...
public class SubsonicDataResponse<T> : IXmlSerializable where T : SubsonicContent<T>, new()
{
    /// <inheritdoc cref="XmlHintSerializable{T}.AttributeHints" path="/summary"/>
    private static readonly IEnumerable<IConvertHint<SubsonicDataResponse<T>>> _ConvertAttributeHints = [
        new StringConvertHint<SubsonicDataResponse<T>>("status", m => m.Status, (m, v) => m.Status = v),
        new StringConvertHint<SubsonicDataResponse<T>>("version", m => m.Version, (m, v) => m.Version = v),
        new StringConvertHint<SubsonicDataResponse<T>>("type", m => m.Type, (m, v) => m.Type = v),
        new StringConvertHint<SubsonicDataResponse<T>>("serverVersion", m => m.ServerVersion, (m, v) => m.ServerVersion = v),
        new BoolConvertHint<SubsonicDataResponse<T>>("openSubsonic", m => m.OpenSubsonic, (m, v) => m.OpenSubsonic = v)
    ];

    /// <inheritdoc cref="XmlHintSerializable{T}.ElementHints" path="/summary"/>
    private static readonly IEnumerable<IConvertHint<SubsonicDataResponse<T>>> _ConvertElementHints = [
        new NestedConvertHint<SubsonicDataResponse<T>, T>(new T().ElementHintName, m => m.Content, (m, v) => m.Content = v),
        new NestedConvertHint<SubsonicDataResponse<T>, SubsonicError>("error", m => m.Error, (m, v) => m.Error = v)
    ];
    ...
    public IDictionary<string, dynamic?>? JsonDataProvider
    {
        ...
        set
        {
            foreach (IConvertHint<SubsonicDataResponse<T>> hint in _ConvertElementHints.Concat(_ConvertAttributeHints))
            {
